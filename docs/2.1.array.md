...
## 1.转字符串 (3)
### 01.转字符串的方法 toString 原数组改变 返回新的字符串
...
const arr2 = arr.sort()
console.log(arr, arr === arr2) // 返回排序之后的原数组 新数组和原数组内存地址一致 元素顺序改变啦
```
...
console.log(arr4, arr4.splice()) // [1, 2, 3, 4, 5] 拷贝全数组
...
> a.forEach 没有返回值 手动写return语句也无效  b. 第二个参数可以改变this执行
```js
const arr = [1, 2, 3, 4]
...
```
> 稀疏数组数组 forEach遍历会跳过数组元素为空的项 for...in for...of 不会
```js
